...
namespace Assisticant.Binding
{
	...
	public static class NumberPickerBindingExtensions
	{
		class ValueBinding<TData> : IInputSubscription
		{
			...
			private IValueConverter<int, TData> _converter;

			public ValueBinding(NumberPicker control, Action<TData> input, IValueConverter<int, TData> converter)
			{
				_control = control;
				_input = input;
		...
		class Identity : IValueConverter<int, int>
		{
		...
		public static void BindValue<TData>(this BindingManager bindings, NumberPicker control, Func<TData> output, Action<TData> input, IValueConverter<int, TData> converter)
		{
